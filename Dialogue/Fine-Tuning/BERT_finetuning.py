...
from transformers.modeling_bert_dot import BertForSequenceClassification
...
class InputFeatures(object):
    def __init__(self, input_ids, input_mask, segment_ids, label, lenidx, last_len, response_len):
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        cur_tensors = (torch.tensor(cur_features.input_ids),
                       ...
                       torch.tensor(cur_features.last_len, dtype=torch.long),
                       torch.tensor(cur_features.response_len, dtype=torch.long)
                       )
...
def convert_examples_to_features(item, train, bert_tokenizer):
    ...
    features = InputFeatures(input_ids=input_ids,
                             ...
                             label=train['y'][item],
                             lenidx=lenidx,
                             last_len=train['sep_len'][item][0],
                             response_len=train['sep_len'][item][1])
    return features


class NeuralNetwork(nn.Module):

    def __init__(self, args, deep_number):
        ...
        self.bert_config = config_class.from_pretrained('/mnt/public/home/lvwp/process/SPM/pytorch_pretrained_bert/bert-base-chinese',num_labels=1)
        self.bert_tokenizer = BertTokenizer.from_pretrained('/mnt/public/home/lvwp/process/SPM/pytorch_pretrained_bert/bert-base-chinese',do_lower_case=args.do_lower_case)
        special_tokens_dict = {'eos_token': '[eos]'}
        num_added_toks = self.bert_tokenizer.add_special_tokens(special_tokens_dict)
        self.bert_model = model_class.from_pretrained('/mnt/public/home/lvwp/process/SPM/pytorch_pretrained_bert/bert-base-chinese',config=self.bert_config, deep_number=self.deep_number)
        #print(self.bert_model) 

        self.bert_model.resize_token_embeddings(len(self.bert_tokenizer))
        """You can load the post-trained checkpoint here."""
        # self.bert_model.bert.load_state_dict(state_dict=torch.load("/mnt/public/home/lvwp/process/UDC_/FP/FP/FPT/PT_checkpoint/ubuntu25/bert.pt"), strict=False)
        self.bert_model.bert.load_state_dict(state_dict=torch.load("/mnt/public/home/lvwp/process/UDC_/FP/FP/FPT/PT_checkpoint/douban27/bert.pt"))
    ...
    def train_step(self, i, data):
        with torch.no_grad():
            batch_ids, batch_mask, batch_seg, batch_y, batch_len, last_len, response_len = (item.cuda(device=self.device) for item in data)

        output = self.bert_model(batch_ids, batch_mask, batch_seg, last_len=last_len, response_len=response_len, deep_number=self.deep_number)
    ...
    def evaluate(self, dev, is_test=False):
        ...
        with open('/mnt/public/home/lvwp/process/UDC_/FP/FP/Fine-Tuning/result/new/dot.txt', "a") as writer:
            writer.write(str(self.deep_number) + ":\t" + str(result[0]) + "\t" + str(result[1]) + "\t" + str(result[2]) + "\t" + str(result[3]) +
    ...
    def predict(self, dev):
        ...
        for i, data in enumerate(dataloader):
            with torch.no_grad():
                batch_ids, batch_mask, batch_seg, batch_y, batch_len, last_len, response_len = (item.cuda() for item in data)
            with torch.no_grad():
                output = self.bert_model(batch_ids, batch_mask, batch_seg, last_len=last_len, response_len=response_len)
