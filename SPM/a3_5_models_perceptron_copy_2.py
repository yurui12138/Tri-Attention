...
class Multil_Atten(nn.Module):
    def __init__(self): #config
        ...
        self.add_dense = nn.Linear(self.attention_head_size, 1).cuda()
        self.add_dense_vt = nn.Linear(30, 30).cuda()
    ...
    def forward(self, sen1, sen2, context_sens=None, attention_mask=None):
        ...
        # attention_scores = self.add_dense_vt(nn.Tanh()(self.add_dense(torch.cat([query_layer, key_layer], dim=-1))))
        query_key_context = unsqueeze(query_layer, dim=3) + unsqueeze(key_layer, dim=2)
        ...
        query_key_context = squeeze(self.add_dense(query_key_context), dim=-1)
        ...
        #attention_probs = attention_probs + context_sens
        ...
        context_layer = squeeze(self.add_dense_v(torch.transpose(context_layer, dim0=-1, dim1=-2)), dim=-1)
