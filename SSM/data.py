...
class DataPrecessForSentence(Dataset):
    ...
    def __init__(self, bert_tokenizer, df, max_word_len = 64):
        ...
        self.seqs_s1, self.seq_masks_s1, \
        self.seqs_s2, self.seq_masks_s2, \
        self.labels = self.get_input(df)
    ...
    def __getitem__(self, idx):
        return self.seqs[idx], self.seq_masks[idx], self.seq_segments[idx], \
               self.seqs_s1[idx], self.seq_masks_s1[idx],\
               self.seqs_s2[idx], self.seq_masks_s2[idx],\
               self.labels[idx]
    ...
    def get_input(self, df):
        ...
        # 获取单句定长序列及其mask
        result_s1 = list(map(self.trunate_and_pad_one, tokens_seq_1))
        seqs_s1 = [i[0] for i in result_s1]
        seq_masks_s1 = [i[1] for i in result_s1]

        result_s2 = list(map(self.trunate_and_pad_one, tokens_seq_2))
        seqs_s2 = [i[0] for i in result_s2]
        seq_masks_s2 = [i[1] for i in result_s2]
        ...
        return torch.Tensor(seqs).type(torch.long), torch.Tensor(seq_masks).type(torch.long),torch.Tensor(seq_segments).type(torch.long), \
               torch.Tensor(seqs_s1).type(torch.long), torch.Tensor(seq_masks_s1).type(torch.long),\
               torch.Tensor(seqs_s2).type(torch.long), torch.Tensor(seq_masks_s2).type(torch.long),\
    ...
    def trunate_and_pad_one(self, tokens_seq):
        '''获得单句的表示'''
        seq_len = 30
        # 对超长序列进行截断，sentence1不超过154，sentence2不超过46
        if len(tokens_seq) > seq_len-2:
            tokens_seq = tokens_seq[0:(seq_len-2)]
        # 分别在首尾拼接特殊符号
        seq = ['[CLS]'] + tokens_seq + ['[SEP]']
        # seq_segment = [0] * (len(tokens_seq) + 2) + [1] * (len(tokens_seq_2) + 1)
        # ID化
        seq = self.bert_tokenizer.convert_tokens_to_ids(seq)
        # 根据max_seq_len与seq的长度产生填充序列
        padding = [0] * (seq_len - len(seq))
        # 创建seq_mask：表明seq长度有意义，padding无意义
        seq_mask = [1] * len(seq) + padding
        # 创建seq_segment
        # seq_segment = seq_segment + padding
